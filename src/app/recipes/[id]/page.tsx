...
export default async function RecipeDetailPage({ params }: Props) {
    ...
    if (!user) return (
        <main className="max-w-3xl mx-auto p-6">
            <p>閲覧にはログインが必要です。</p>
            <a className="px-3 py-2 rouded bg-black text-white inline-block mt-3" href="/auth/login">GitHubでログイン</a>
        </main>
    );

    const { data: recipe, error } = await supabase.from("recipes").select("id, title,cook_time_min, ingredients, steps, tools, created_at").eq("id", params.id).single();
